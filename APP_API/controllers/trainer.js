  ...
  if (!userId) {
    ...
    trainerModel
      ...
      .exec((err, allTrainers) => {
        if (!allTrainers) {
          res.status(500).json({ message: "No trainers found." });
          return;
        } else {
...
const trainersReadOne = function (req, res) {
  if (req.params.trainerid) {
    trainerModel
      ...
      .exec((err, trainer) => {
        //trainer ---- The data which we are fetching from database. it is an object
        if (!trainer) {
          res.status(404).json({ message: "trainer id is not found." });
          return;
        } else if (err) {
...
const trainersUpdateOne = function (req, res) {
  if (req.params && req.params.trainerid) {
    trainerModel.findById(req.params.trainerid).exec((err, trainer) => {
      if (!trainer) {
  ...
  trainerModel
    ...
    .exec((err, allTrainerWithNoStatus) => {
      if (!allTrainerWithNoStatus) {
        res.status(200).json({ message: "No pending requests to approve it." });
        return;
      } else {
        res.status(200).json(allTrainerWithNoStatus);
      }
    });
};
